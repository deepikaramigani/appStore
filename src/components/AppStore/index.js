...
class AppStore extends Component {
  state = {input: '', activeTab: tabsList[0].tabId}
  ...
  render() {
    const {input, activeTab} = this.state
    const activeTabResults = appsList.filter(
      eachApp => eachApp.category === activeTab,
    )
    const searchResults = activeTabResults.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(input.toLowerCase()),
    )
